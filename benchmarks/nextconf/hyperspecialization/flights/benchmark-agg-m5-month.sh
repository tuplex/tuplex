#!/usr/bin/env bash
# version of the C++ experiment using STL hashmap instead of optimized lookup table
# configure variables here

# use only year 2013 to save time
file="/data/flights_on_time_performance_2003_04.csv"
RES_ROOT_DIR=results-agg-experiment-month
NUM_RUNS="${NUM_RUNS:-11}"
SO_SPECIALIZED=agg_weather_specialized-month.so
SO_GENERAL=agg_weather_general-month.so
...
clang++ -shared -fPIC -O3 -march=haswell -msse4.2 -mcx16 -DNDEBUG -o $SO_GENERAL src/agg_query_month/agg_general.cc
clang++ -shared -fPIC -O3 -march=haswell -msse4.2 -mcx16 -DNDEBUG -o $SO_SPECIALIZED src/agg_query_month/agg_specialized.cc
