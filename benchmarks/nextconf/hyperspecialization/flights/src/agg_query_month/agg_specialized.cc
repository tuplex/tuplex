...
static std::unordered_map<int64_t, int64_t> agg_map;
...
int to_day_of_year(int day, int month, int year) {
    ...
    // replace all of this by specialized version for 2003/04
    return 31 + 28 + 31 + day - 1; // not a leap year
...
extern "C" int64_t fetch_aggregate(void *userData, uint8_t** buf, size_t* buf_size) {
    ...
    for(auto keyval : agg_map) {
        ...
        // assume everything has been constant-folded!
        double mean = 0.0;
        double std = 0.0;
