...
RESDIR=experimental_results/github
...
### 1a. Hyper w. filter promotion ###
echo "Benchmark hyper w. filter promotion"
# hyper-specialized
echo "benchmarking hyper (hot)"
for ((r = 1; r <= NUM_RUNS; r++)); do
  LOG="${RESDIR}/github-hyper-run-$r.txt"
  echo "running $r/${NUM_RUNS}"
  timeout $TIMEOUT $PYTHON runtuplex-new.py >$LOG 2>$LOG.stderr
  # copy temp aws_job.json result for analysis
  cp aws_job.json ${RESDIR}/"github-hyper-run-$r.json"
done
# mv job folder
mkdir -p $RESDIR/github-hyper
mv job/*.json $RESDIR/github-hyper
rm -rf job

### 1b. Hyper w. filter promo ###
# hyper-specialized
echo "Benchmark hyper without filter promotion"
for ((r = 1; r <= NUM_RUNS; r++)); do
  LOG="${RESDIR}/github-hyper-nopromo-run-$r.txt"
  echo "running $r/${NUM_RUNS}"
  timeout $TIMEOUT $PYTHON runtuplex-new.py --no-promo >$LOG 2>$LOG.stderr
  # copy temp aws_job.json result for analysis
  cp aws_job.json ${RESDIR}/"github-hyper-nopromo-run-$r.json"
done
# mv job folder
mkdir -p $RESDIR/github-hyper-nopromo
mv job/*.json $RESDIR/github-hyper-nopromo
rm -rf job
