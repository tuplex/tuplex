...
# AWS SDK v1.9+
# because 1.9.5 is broken, force version to 1.9.2
# update as need later...
cd /tmp &&
  git clone --recurse-submodules https://github.com/aws/aws-sdk-cpp.git &&
  cd aws-sdk-cpp && git checkout tags/1.9.39 && mkdir build && pushd build &&
  cmake -DCMAKE_BUILD_TYPE=Release -DBUILD_SHARED_LIBS=OFF -DBUILD_ONLY="s3;core;lambda;transfer" -DCMAKE_INSTALL_PREFIX=/opt -DENABLE_TESTING=OFF .. &&
  make -j32 &&
  make install &&
  popd &&
  cd -
