...
namespace tuplex {
    ...
    PythonPipelineBuilder::PythonPipelineBuilder(const std::string &funcName) : _funcName(funcName), _indentLevel(0),
                                                                                _lastInputRowName(inputRowName()),
                                                                                _lastRowName("row"), _envCounter(0),
                                                                                _parseCells(false),
                                                                                _pipelineDone(false) {
    ...
    void PythonPipelineBuilder::cellInput(int64_t operatorID, std::vector<std::string> columns,
        ...
        if (typeHints.empty()) {
            ...
            writeLine("parsed_row = [parse(el) for el in " + lastInputRowName() + "] if parse_cells else list(" +
                      lastInputRowName() + ")\n");
        } else {
            // allocate None array and then fill in
            writeLine("parsed_row = [None] * " + std::to_string(numColumns) + " if parse_cells else list(" +
                      lastInputRowName() + ")\n");
            ...
            for (size_t i = 0; i < numColumns; ++i) {
                ...
                if (it == typeHints.end())
                    writeLine("parsed_row[" + std::to_string(i) + "] = parse(" + lastInputRowName() + "[" +
                              std::to_string(i) + "])\n");
                else {
                    ...
                    if (it->second.isOptionType()) {
                        writeLine(
                                "if " + lastInputRowName() + "[" + std::to_string(i) + "] in " + vecToList(na_values) +
                                ":\n");
                    ...
                    if (python::Type::STRING == t) {
                        writeLine("parsed_row[" + std::to_string(i) + "] = " + lastInputRowName() + "[" +
                                  std::to_string(i) + "]\n");
                    } else if (python::Type::BOOLEAN == t) {
                        writeLine("parsed_row[" + std::to_string(i) + "] = to_bool(" + lastInputRowName() + "[" +
                                  std::to_string(i) + "].strip())\n");
                    } else if (python::Type::I64 == t) {
                        writeLine("parsed_row[" + std::to_string(i) + "] = int(" + lastInputRowName() + "[" +
                                  std::to_string(i) + "].strip())\n");
                    } else if (python::Type::F64 == t) {
                        writeLine("parsed_row[" + std::to_string(i) + "] = float(" + lastInputRowName() + "[" +
                                  std::to_string(i) + "].strip())\n");
                    } else {
                        throw std::runtime_error(
                                "unsupported type hint " + it->second.desc() + " found in python codegen. Abort.");
                    }
        ...
        if (!projectionMap.empty()) {
            ...
            writeLine(
                    "projected_row = [None] * " + std::to_string(numColumns) + "\n"); // fill with None as dummy element
            // project elements & column names
            for (auto keyval: projectionMap)
                writeLine("projected_row[" + std::to_string(keyval.first) + "] = parsed_row[" +
                          std::to_string(keyval.second) + "]\n");
            if (!columns.empty()) {
    ...
    void PythonPipelineBuilder::mapOperation(int64_t operatorID, const tuplex::UDF &udf,
    ...
    void PythonPipelineBuilder::withColumn(int64_t operatorID, const std::string &columnName, const tuplex::UDF &udf) {
        ...
        code << "call_res = apply_func(f, " + row() + ")\n"
             ...
             << "\t" << row() << ".data = tuple(tmp)\n"
             << "else:\n"
             << "\t" << row() << ".columns = " << row() << ".columns + ('" << columnName << "',) if " << row()
             << ".columns is not None else tuple([None] * len(" << row() << ".data)) + ('" << columnName << "',)\n"
...
#warning "type check is missing here! should be done based on finalOutputType!!! This is important when using e.g. sinks like ORC, parquet, etc. - everything that has structured types."
    ...
    void PythonPipelineBuilder::innerJoinDict(int64_t operatorID, const std::string &hashmap_name,
        ...
        exceptInnerCode(code, operatorID,
                        "Exception('INTERNAL: key column \"" + leftColumn.value() + "\" not in " + row() + "')", row(),
                        1);
        ...
        code << "\tkey_column" << envSuffix() << " = " << row() << ".columns[col_idx" << envSuffix() << "] if " << row()
             << ".columns else None\n";
        ...
        code << "\t\ttmp_data = tuple(left_tmp" << envSuffix() << "[:col_idx" << envSuffix() << "] + left_tmp"
             << envSuffix() << "[col_idx" << envSuffix() << "+1:] + [key" << envSuffix() << "] + right_tmp"
             << envSuffix() << ")\n";
        code << "\t\ttmp_columns = tuple(left_cols" << envSuffix() << "[:col_idx" << envSuffix() << "] + left_cols"
             << envSuffix() << "[col_idx" << envSuffix() << "+1:] + [left_cols" << envSuffix() << "[col_idx"
             << envSuffix() << "]] + right_cols" << envSuffix() << ")\n";
    ...
    void PythonPipelineBuilder::flushLastFunction() {
        ...
        if (_lastFunction._handlers.empty()) {
            ...
            for (int i = 0; i < _lastFunction._handlers.size(); ++i) {
                ...
                writeLine("if 'exception' in res.keys() and issubclass(type(res['exception']), " +
                          exceptionCodeToPythonClass(ecCode) + "):\n");
                indent();
...
//            dedent();
    ...
    void PythonPipelineBuilder::pythonAggByKey(int64_t operatorID,
        ...
        ss << "\tagg_value = " << hashmap_name << ".setdefault(agg_key, result_to_row("
           << initial_value.toPythonString() << "))\n";
    ...
    std::string codegenPythonCombineAggregateFunction(const std::string &function_name,
        ...
        switch (agg_type) {
            ...
            default:
                throw std::runtime_error(
                        "unknown aggregate type " + std::to_string(static_cast<int>(agg_type)) + " encountered.");
        }
