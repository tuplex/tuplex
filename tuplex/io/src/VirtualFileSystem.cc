...
#ifdef BUILD_WITH_AWS
    VirtualFileSystemStatus VirtualFileSystem::addS3FileSystem(const std::string& access_key, const std::string& secret_key, const std::string& region, const std::string &caFile, bool lambdaMode, bool requesterPay) {
        auto impl = new S3FileSystemImpl(access_key, secret_key, region, caFile, lambdaMode, requesterPay);
        return VirtualFileSystem::registerFileSystem(std::make_shared<S3FileSystemImpl>(access_key, secret_key, region, caFile, lambdaMode, requesterPay), "s3://");
    }
