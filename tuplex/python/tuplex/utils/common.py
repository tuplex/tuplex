...
def is_shared_lib(path: str) -> bool:
    ...
    return (
        ...
        or mime_type == "application/x-application"
    )
...
def logging_callback(level: int, time_info: str, logger_name: str, msg: str) -> None:
    ...
    log_record = logging.LogRecord(
        logger_name, level, pathname, lineno, msg, None, None
    )
...
def check_mongodb_connection(
    ...
    while abs(time.time() - start_time) < timeout:
        ...
        try:
            ...
            client = MongoClient(
                uri, serverSelectionTimeoutMS=100, connectTimeoutMS=1000
            )
            client.server_info()  # force a call to mongodb, alternative is client.admin.command('ismaster')
            connect_successful = True
        except Exception as e:
            logging.debug(
                "Connection try {} produced {} exception {}".format(
                    connect_try, type(e), str(e)
                )
            )
        ...
        logging.debug(
            "Contacting MongoDB under {}... -- {:.2f}s of poll time left".format(
                uri, timeout - (time.time() - start_time)
            )
        )
        connect_try += 1
    ...
    if connect_successful is False:
        raise Exception(
            "Could not connect to MongoDB, check network connection. (ping must be < 100ms)"
        )
...
def find_or_start_mongodb(
    ...
    if "localhost" in mongodb_url:
        ...
        if not cmd_exists("mongod"):
            raise Exception(
                "MongoDB (mongod) not found on PATH. In order to use Tuplex's WebUI, you need MongoDB"
        ...
        if is_process_running("mongod"):
            ...
            try:
                ...
                logging.debug(
                    "starting MongoDB daemon process via {}".format(" ".join(cmd))
                )
                ...
                try:
                    ...
                    logging.info(
                        "Could not start MongoDB daemon process in {}s, trying with timeout={}s".format(
                            short_timeout, max_mongodb_timeout
                        )
                    )
                    p_stdout, p_stderr = process.communicate(
                        timeout=max_mongodb_timeout
                    )
                ...
                if len(p_stderr.strip()) > 0:
                    raise Exception(
                        "mongod produced following errors: {}".format(p_stderr)
                    )

                # find mongod pid
                m = re.search(r"forked process: (\d+)", p_stdout)
                assert m is not None, (
                    "Could not find Child process ID when starting MongoDB"
                )
                mongo_pid = int(m[1])
                ...
                logging.error(
                    "Failed to start MongoDB daemon. Details: {}".format(str(e))
                )
                ...
                if os.path.isfile(mongodb_logpath):
                    with open(mongodb_logpath, "r") as fp_mongo:
                        lines = list(
                            map(lambda line: line.strip(), fp_mongo.readlines())
                        )
                        shortened_log = ""
                    ...
                    logging.error(
                        "Could not find MongoDB log under {}. Permission error?".format(
                            mongodb_logpath
                        )
                    )
...
def log_gunicorn_errors(logpath: str) -> None:
    ...
    with open(logpath, "r") as fp:
        ...
        indices = map(
            lambda t: t[1],
            filter(lambda t: "[ERROR]" in t[0], zip(lines, range(len(lines)))),
        )
        if indices:
...
def find_or_start_webui(
    ...
    """
    version_endpoint = (
        "/api/version"  # use this to connect and trigger WebUI connection
    )
    ...
    try:
        ...
        logging.debug(
            "Couldn't connect to {}, starting WebUI...".format(
                base_uri + version_endpoint
            )
        )
    ...
    if version_info is not None:
        ...
        assert dir_path.endswith(os.path.join("tuplex", "utils")), (
            "folder structure changed. Need to fix."
        )
        # get tuplex base dir
        ...
        if not os.path.isdir(os.path.join(tuplex_basedir, "historyserver", "thserver")):
            ...
            while path.parent != path:
                # check in path
                if "tuplex" in os.listdir(path) and "historyserver" in os.listdir(
                    os.path.join(path, "tuplex")
                ):
                    tuplex_basedir = os.path.join(str(path), "tuplex")
                    logging.debug(
                        "Detected Tuplex rootfolder (dev) to be {}".format(
                            tuplex_basedir
                        )
                    )
                    break
        ...
        assert os.path.isdir(
            os.path.join(tuplex_basedir, "historyserver", "thserver")
        ), "could not find Tuplex WebUI WebApp in {}".format(tuplex_basedir)
        assert os.path.isfile(
            os.path.join(tuplex_basedir, "historyserver", "thserver", "__init__.py")
        ), "could not find Tuplex WebUI __init__.py file in thserver folder"
        ...
        gunicorn_host = "{}:{}".format(
            hostname.replace("http://", "").replace("https://", ""), port
        )
        ...
        try:
            ...
            logging.error(
                "ensuring parent dir of {} exists, failed with {}".format(
                    web_logfile, e
                )
            )
        ...
        process = subprocess.Popen(
            cmd, stdout=subprocess.PIPE, stderr=subprocess.PIPE, env=ui_env
        )
        # set a timeout of 2 seconds to keep everything interactive
        ...
        while time.time() - start_time < TIME_LIMIT:
            if not os.path.isfile(PID_FILE) or os.stat(PID_FILE).st_size == 0:
                ...
                break
            logging.debug(
                "Polling for Gunicorn PID... -- {:.2f}s of poll time left".format(
        ...
        try:
            ...
            raise Exception(
                "Failed to start gunicorn daemon, non-daemon run yielded:\n{}".format(
                    non_daemon_log
                )
            )
            ...
            for proc in psutil.process_iter():
                try:
                    ...
                    if "gunicorn" in sep_line:
                        ...
                        if (
                            "thserver:app" in proc.cmdline()
                            and gunicorn_host in proc.cmdline()
                            and PID_FILE in proc.cmdline()
                        ):
...
def ensure_webui(options: dict) -> None:
    ...
    assert options["tuplex.webui.enable"] is True, (
        "only call ensure webui when webui option is true"
    )
    ...
    mongodb_logpath = os.path.join(
        options["tuplex.scratchDir"], "webui", "logs", "mongod.log"
    )
    gunicorn_logpath = os.path.join(
        options["tuplex.scratchDir"], "webui", "logs", "gunicorn.log"
    )
    ...
    try:
        ...
        find_or_start_mongodb(
            mongodb_url, mongodb_port, mongodb_datapath, mongodb_logpath
        )
        ...
        version_info = find_or_start_webui(
            mongo_uri, webui_url, webui_port, gunicorn_logpath
        )
        ...
        if "dev" not in __version__ and version_info["version"] != __version__:
            logging.warning(
                "Version of Tuplex WebUI ({}) and Tuplex ({}) do not match.".format(
