# This is code schedule to be part of Python 3.14+ from https://github.com/python/cpython/pull/122946/files, ported back here for usagge to avoid pyarrow bug
# where AWS SDK is always initialized and thus may lead to issues with tuplex.
...
if os.name == "nt":
    ...
    def _get_build_version() -> Union[None, int, float]:
    ...
    def _get_module_handles() -> List[Any]:
    ...
    def dllist() -> List[str]:
        ...
        libraries = [
            name for h in modules if (name := _get_module_filename(h)) is not None
        ]
        return libraries
    ...
    def _findLib_gcc(name: str) -> Optional[str]:
    ...
    if sys.platform == "sunos5":
        ...
        def _get_soname(f: Any) -> Optional[str]:
            if not f:
                return None
    ...
    if sys.platform.startswith(("freebsd", "openbsd", "dragonfly")):
        ...
        def _num_version(libname: str) -> List[int]:
        ...
        def _findLib_crle(name: str, is64: bool) -> Optional[str]:
        ...
        def find_library(name: str, is64: bool = False) -> Optional[str]:
            return _get_soname(_findLib_crle(name, is64) or _findLib_gcc(name))

    else:

        def _findSoname_ldconfig(name: str) -> Optional[str]:
            import struct
        ...
        def _findLib_ld(name: str) -> Optional[str]:
...
if os.name == "posix" and sys.platform not in {"darwin", "ios", "tvos", "watchos"}:
    ...
    if hasattr((_libc := ctypes.CDLL(None)), "dl_iterate_phdr"):
        ...
        @_dl_phdr_callback
        def _info_callback(info, _size, data) -> int:  # noqa: ANN001
