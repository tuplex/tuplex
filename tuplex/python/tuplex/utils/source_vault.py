    ...
    try:
        ...
        if sys.version_info.major >= 3 and sys.version_info.minor >= 9:
            ...
            try:
                ...
                logging.debug(
                    "gen_code_for_lambda via ast (python3.9+) failed with {}".format(e)
                )
        ...
        if filename and lineno:
            ...
            if len(entries) == 1:
                ...
                if hasattr(ftor.__code__, "co_firstcolno"):
                    ...
                    if len(globs) != 0:
                        raise KeyError(
                            "Multiple lambdas found in {}:+{}, can't extract source code for "
                    ...
                    for entry in entries:
                        if entry["code_hash"] == codeobj_hash:
                            return entry["code"]
                raise KeyError(
                    "Multiple lambdas found, but failed to retrieve code for this lambda expression."
                )
        else:
    ...
    def extractAndPutAllLambdas(
        ...
        if lam_count_in_target_line != 1:
            ...
            if len(globals) != 0 and not supports_lambda_closure():
                raise Exception(
                    "Found {} lambda expressions in {}:{}. Please patch your interpreter or "
                    "reformat so Tuplex can extract the source code.".format(
                        lam_count_in_target_line, os.path.basename(filename), lineno
                    )
                )
            else:
        ...
        if len(Lams) == 1:
            ...
            if key in self.lambdaFileDict.keys():
                ...
                existing_entries = self.lambdaFileDict[
                    key
                ]  # how many can there be? assume 1 at most!
                updated_existing = False
                for i, existing_entry in enumerate(existing_entries):
                    if (
                        existing_entry["code"] == entry["code"]
                        and existing_entry["code_hash"] == entry["code_hash"]
                        and existing_entry["colno"] == entry["colno"]
                    ):
                        self.lambdaFileDict[key][i] = (
                            entry  # update entry in existing file/lineno dict
                        )
            ...
            if colno is None and len(globals) != 0:
                raise Exception(
                    "Found more than one lambda expression on {}:+{}. Either use "
            ...
            for lam in Lams:
                ...
                if colno is None:  # interpreter not patched
                    assert len(globals) == 0, (
                        "this path should only be taken if there are no globs"
                    )
